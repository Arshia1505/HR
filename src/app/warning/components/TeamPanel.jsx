...
export default function TeamPanel({ team, onClose, onUploadCSV, onAddMember, onSendWarnings }) {
  ...
  if (!team)
    return <section className="text-center text-gray-300">Select a team to see details and manage members.</section>;
  ...
  return (
    <section className="bg-black/40 backdrop-blur-md border border-white/20 rounded-lg p-4 text-white">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-xl font-semibold">{team.name}</h2>
        ...
        <div className="flex gap-2">
          <label className="inline-flex items-center gap-2 bg-white/10 px-3 py-2 rounded cursor-pointer">
            <input type="file" accept=".csv" className="hidden" onChange={e => onUploadCSV(team.id, e.target.files[0])} />
            <span className="text-sm">Upload CSV</span>
          </label>

          <button
            className="px-3 py-2 bg-orange-600 hover:bg-orange-700 rounded transition"
            onClick={handleSendSelected}
            disabled={sending}
            title="Send warning emails to selected members"
          >
          ...
          <button className="px-3 py-2 bg-red-700 hover:bg-red-800 rounded" onClick={onClose}>
            Close
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-medium">Members ({members.length})</h3>
            <label className="text-sm inline-flex items-center gap-2">
              <input type="checkbox" checked={selectAll} onChange={toggleSelectAll} className="accent-orange-500" />
          ...
          <div className="space-y-2">
            {members.map(m => (
              <div key={m.id} className="flex items-center justify-between p-3 border border-white/10 rounded bg-white/5">
                <div className="flex items-center gap-3">
                  <input
                    type="checkbox"
                    checked={selectedIds.includes(m.id)}
                  ...
                  <div>
                    <div className="font-medium">{m.name}</div>
                    <div className="text-xs text-gray-300">{m.role} • {m.email}</div>
                  </div>
                </div>

                <div className="flex items-center gap-2">
                  <button
                    className="text-sm px-2 py-1 bg-yellow-600 hover:bg-yellow-700 text-black rounded"
                    onClick={async () => { setSelectedIds([m.id]); await handleSendSelected(); }}
                    disabled={sending}
                  >
                    Warn
                  </button>
        ...
        <div>
          ...
          <div className="mt-6">
            <h4 className="font-medium mb-2">CSV format</h4>
            <p className="text-sm text-gray-300">Header row: name,email,role (optional id)</p>
            <pre className="mt-2 p-2 bg-white/10 text-xs rounded">name,email,role{'\n'}John Doe,john@example.com,Volunteer</pre>
          </div>
        </div>
      </div>

      {statusMessage && (
        <div className={`mt-4 p-3 rounded ${statusMessage.type === 'error' ? 'bg-red-700 text-red-200' : 'bg-green-700 text-green-200'}`}>
          {statusMessage.text}
        </div>
      )}
    </section>
